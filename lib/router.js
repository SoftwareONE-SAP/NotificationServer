...
function Router() {
	 ...
	 * @type {Number}
	 */
	this._state = Router.States.stopped;
...
/**
 * States for running state of Router
 * @type {Object}
 */
Router.States = {
	/**
...
Router.prototype.attach = function() {
	for (var i = 0, j = arguments.length; i < j; i++) {
		switch(arguments[i].constructor.super_.name.toLowerCase()) {
			case 'baseinput':
				this.registerInput(arguments[i]);
			break;
			case 'baseprocessor':
				this.registerProcessor(arguments[i]);
			break;
			case 'baseoutput':
				this.registerOutput(arguments[i]);
			break;
			default:
				throw new Error("Invalid class {" + arguments[i].constructor.super_.name + "}")
			break;
		}
	}
};
 ...
 */
Router.prototype.handleInputNotification = function(notification) {
	/**
 ...
 */
Router.prototype.registerInput = function(input) {
	if(this._state !== Router.States.stopped)
 ...
 */
Router.prototype.registerOutput = function(output) {
	if(this._state !== Router.States.stopped)
 ...
 */
Router.prototype.registerProcessor = function(processor) {
	if(this._state !== Router.States.stopped)
 ...
 * Sort processors
 */
Router.prototype._sortProcessors = function() {
	var t = new Toposort();
 ...
 */
Router.prototype._createProcessCallback = function(processor, notification) {
...
Router.prototype.start = function() {
	if(this._state !== Router.States.stopped) {
		throw new Error("Router is currently started.");
	}

	/**
	 * Set the state to pending
	 */
	this._state = Router.States.starting;
